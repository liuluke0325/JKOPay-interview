...
| M5  | `/search` route: input + abort + debounce + loading + empty + tabbed results + restore-on-cancel                                         | done     |
| M6  | `/items/[id]` detail page: category-specific mock fields + back-with-scroll-restore (scroll-restore deferred — user scope cut 2026-05-03) | done     |
| M7  | Tests: unit (API + cursor + restore logic) + e2e (scroll, tab switch, search→empty, cancel-restore, detail nav) + ItemList tests (per Codex RR-005: onRowsRendered fetchNextPage trigger near end, no duplicate pages while fetching, empty/error/end-of-list states) + SearchClient tests (per Codex RR-006: debounce-then-URL sync, AbortSignal propagation across rapid q changes, cancel restore happy + stale paths, deep-link `?q=` populates input, browser back/forward syncs input from URL, `consumeRestoreState` atomic clear, **browser-back-from-/search loses scroll** — Codex RR-006 Reconfirmation 2/3 carry-over: SearchClient consumes on mount + only re-saves on 取消; verify M6 detail back-nav does not reuse the same pattern blindly) | pending  |
| M8  | Deploy demo: Railway-only (Postgres + Dockerfile-BE + Dockerfile-FE in one project)                                                      | done (BE active, FE redeploying for NEXT_PUBLIC_API_BASE_URL fix) |
| M9  | Final prep: README (zh-TW, incl. AI 使用聲明) + 15 ADRs in place + `docs/prompts/` curated 3 sessions                                       | done (CORS lock pending FE smoke) |
