...
- **Phase**: A complete (scaffold aligned to brief). Ready to start M1.
- **Last updated**: 2026-05-02
- **Branch**: main (no git repo yet — will `git init` before first code commit)
...
| M1  | BE skeleton: Fastify + TS + Prisma `Item` schema (`category` + `subCategory` enums + nullable category-specific fields) + Postgres + seed (≥90 items, real logos for some) | pending |
