...
| R3  | Search latency at production scale — `ILIKE '%q%'` is a seq scan that doesn't use btree indexes | M      | H      | At 10k+ items + 100 RPS the existing btree indexes on `title`/`description` are useless for leading-`%` substring search; planner falls back to a full table scan, DB CPU saturates. **Mitigated by the M2-hardening migration that enables `pg_trgm` + adds GIN indexes (`gin_trgm_ops`) on title/description.** See `docs/SCALING.md` for FTS as an alternative path. |
...
| R13 | DB connection pool exhaustion at 1000s concurrent BE clients                   | M      | H      | Prisma defaults to ~10 connections; under load this queues. Mitigated by `DATABASE_URL?connection_limit=N` env-driven tuning + a single shared `prisma` client (already done). For real scale put PgBouncer in front; documented in `docs/SCALING.md`. |
| R14 | JSON-with-Chinese payload size at scale — egress + slow mobile clients         | M      | M      | `@fastify/compress` (br + gzip, threshold 1024) — verified ~10× reduction on the 30-item Chinese list payload. `Cache-Control` per route lets CDN/edge serve repeat hits without hitting origin. Implemented in M2 hardening pass. ETag is **deliberately not** installed — Cache-Control covers the access pattern; ETag rationale and the path to add it later are documented in `docs/SCALING.md`. |
| R15 | Single bad client can exhaust BE / DB resources                                | M      | H      | `@fastify/rate-limit` (env-tunable, 100 req/min/IP default). **`trustProxy` defaults OFF** so `req.ip` is the unspoofable socket peer; production deployments behind an LB must set `TRUST_PROXY` to a CIDR list or hop count matching the upstream topology — see `docs/SCALING.md`. (Codex caught spoofability in RR-003 review when `trustProxy: true` was the default.) For real production: per-route + auth-tier limits + Redis-backed state across replicas; deferred and documented. |
